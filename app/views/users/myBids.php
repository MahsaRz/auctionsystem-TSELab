<?php require APPROOT . "/views/inc/header.php"; ?>
    ...
    <main role="main">
    <?php echo empty($data['items']) ? '<div>No Bids Made Yet!</div>' : '' ?>
                    ...
                    <div class="col-md-4" id="<?php echo $item->auctionID ?>Card">
                        <div class="card mb-4 shadow-sm">
                            <div class="text-center">
                                <img style="height: 250px; width: auto;" class="mx-auto m-3 rounded img-thumbnail" src="<?php echo URLROOT; ?>/public/img/<?php echo $item->imageFileName; ?>">
                            </div>
                            <hr class="m-1">
                            <div class="card-body">
                                ...
                                <p class="amountSlot text-info" id="<?php echo $item->auctionID ?>Amount"><?php echo $item->isActive == 1 ? "Highest Bid: $" : "Sold At: $"; ?><?php echo $item->highestBid; ?></p>
                                <p class="yourBid text-info">
                                    Your Bid: $<?php echo $item->bidAmount?>
                                    <?php if($item->isActive == 1) { ?>
                                        <?php echo $item->highestBid > $item->bidAmount ? '<span class="text-danger">&emsp;You are lagging behind!</span>' : '<span class="text-success">&emsp;You are leading the bid!</span>'?>
                                    <?php } elseif($item->isActive == 0) {?>
                                        <?php echo $item->highestBid > $item->bidAmount ? '<span class="text-danger">&emsp;You lost the bid!</span>' : '<span class="text-success">&emsp;You won the bid!</span>'?>
                                    <?php } ?>
                                </p>
                ...
                success: function(result){
                    ...
                    $.each(result, function(index, value){
                        if(value[2] == 1){
                            ...
                            $('#'.concat(value[0]).concat("Card")).show();
                            document.getElementById((value[0]).concat("Amount")).innerHTML = "Sold At: $".concat(value[5]);
                        }
                    });
                }
            });
